...
[![Version](https://img.shields.io/badge/version-1.2.5-blue.svg)](https://github.com/muskspace0806-prog/Log-interception)
...
[![CocoaPods](https://img.shields.io/badge/pod-1.2.5-blue.svg)](https://cocoapods.org/pods/ZWB_LogTap)
...
```ruby
# 仅在 Debug 模式下使用
pod 'ZWB_LogTap', '~> 1.2.5', :configurations => ['Debug']
```
...
- 🐛 老用户版本迁移：自动补充新默认规则（不覆盖用户已有规则）

### [1.2.3] - 2026-03-25

#### Added
- ✅ IM 列表消息行优先显示 `Route: roomMicCharmPush` 格式，字体加大加粗（15pt semibold）
- ✅ IM 过滤支持 route 字段匹配（过滤列表加 `heartbeat` 可同时过滤 IM 心跳消息）
