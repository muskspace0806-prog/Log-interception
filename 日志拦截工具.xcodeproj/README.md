...
2. **InterceptedRequest.swift** - 拦截请求数据模型
3. **NetworkInterceptorManager.swift** - 拦截管理器

### UI 文件
4. **FloatingButton.swift** - 可拖拽悬浮按钮
5. **NetworkLogViewController.swift** - 日志列表页面
6. **NetworkLogCell.swift** - 日志列表单元格
7. **NetworkLogDetailViewController.swift** - 日志详情页面
...
### 日志列表功能
- 搜索框：搜索 URL、请求、响应内容
- 过滤器：按请求方法（GET/POST）或状态（成功/失败）过滤
- 点击列表项：查看详细信息
- 清空按钮：清空所有日志
- 导出按钮：导出日志为 JSON 格式
...
### 无法拦截
❌ CFNetwork 直接调用
❌ 原始 Socket 连接（需要 Network Extension）
❌ 系统级别的网络请求

## 高级功能（可选）

### 拦截 Socket 连接
如果需要拦截 Socket 层的连接，需要使用 Network Extension：
1. 添加 Network Extension 权限
2. 实现 NEPacketTunnelProvider
3. 需要用户授权 VPN 权限
